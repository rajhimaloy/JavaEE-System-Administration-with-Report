...
@Named
@ViewScoped
public class UserActiveInactiveController implements Serializable {
	
	private static final long serialVersionUID = 1L;
	
	@Inject
	private IUserActiveInactiveService iUserActiveInactiveService;	
	...
	@RequiresPermissions("10010006:UPDATE")
	public void update() {		
		
		try {
			userInfo = iUserSetupService.getUserSetupDetails(userInfoDTO.getUserName());
			
			String messageDetail = iUserActiveInactiveService.update(userInfoDTO, userInfo);
